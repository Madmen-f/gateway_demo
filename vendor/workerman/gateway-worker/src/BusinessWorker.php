        ...
        switch ($cmd) {
            case GatewayProtocol::CMD_ON_CONNECT:
                if ($this->_eventOnConnect) {
                    ...
                    \GatewayWorker\Lib\Log::write("[connect]" . "client_id:" . Context::$client_id . "连接", Log::INFO);
                    \GatewayWorker\Lib\Log::setLogType(2);
                    \GatewayWorker\Lib\Log::write("client_id:" . Context::$client_id . "连接", Log::INFO);
                }
                break;
            case GatewayProtocol::CMD_ON_MESSAGE:
                $messge = json_decode($data['body'],true);
                ...
                if (is_callable($this->eventHandler . '::' . $this->eventAction)) {
                    ...
                    \GatewayWorker\Lib\Log::write("===== " . "Begin at " . date("Y-m-d H:i:s", time()) . " =====", Log::INFO);
                    \GatewayWorker\Lib\Log::write("client_id is" . Context::$client_id, Log::INFO);
                ...
                } elseif ($messge['handler'] && $messge['actions']) { //相当于404
                    \GatewayWorker\Lib\Log::setLogType(4);
                    \GatewayWorker\Lib\Log::write(Context::$client_id . "没有找到对应的方法" . print_r($messge, true));
                    $error_msg = array(
                        "error_code" => 1,
                        "error_msg" => "没有找到方法",
                    );
                    Gateway::sendToClient(Context::$client_id, json_encode($error_msg));
                } elseif($data['body'] == 'PING') { //心跳
                    //echo "被ping了";
                } else { //让你瞎几把发
            ...
            case GatewayProtocol::CMD_ON_CLOSE:
                ...
                if ($this->_eventOnClose) {
                    ...
                    \GatewayWorker\Lib\Log::setLogType(0);
                    \GatewayWorker\Lib\Log::write("[close]" . "client_id:" . Context::$client_id . "断开连接", Log::INFO);
                    \GatewayWorker\Lib\Log::setLogType(3);
                    \GatewayWorker\Lib\Log::write("client_id:" . Context::$client_id . "断开连接", Log::INFO);
                }
                break;
